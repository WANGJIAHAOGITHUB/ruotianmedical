package cn.ekgc.medical.system.division.controller;
 ...
 * @since 1.0.0
 */
@Api(value = "若天医疗系统信息 - 行政区划信息控制层", tags = "若天医疗系统信息 - 行政区划信息控制层")
...
public class DivisionController extends BaseController {
	...
	public ResponseVO getListByPage(@PathVariable("pageNum") Integer pageNum, @PathVariable("pageSize") Integer pageSize, @RequestBody DivisionVO vo) throws Exception {
		// 先将参数封装为为一个传输对象
		QueryPageVO<DivisionVO> queryPageVO = new QueryPageVO<>(vo, pageNum, pageSize);
		// 调用传输层传入后端
		PageVO<DivisionVO> pageVO = divisionTransport.queryByPage(queryPageVO);
	 ...
	 * @throws Exception
	 */
	@ApiOperation(value = "根据 id 查询行政区划对象信息", httpMethod = "GET", protocols = "application/json")
	@GetMapping ("/id/{id}")
	public ResponseVO getById(@PathVariable("id") String id) throws Exception {
		if (!StrUtil.isBlank(id)) {
			DivisionVO vo = divisionTransport.getById(id);
	 ...
	 * @throws Exception
	 */
	@ApiOperation(value = "根据 code 查询行政区划对象信息", httpMethod = "GET", protocols = "application/json")
	@GetMapping ("/code/{code}")
	public ResponseVO getByCode(@PathVariable("code") String code) throws Exception {
		if (!StrUtil.isBlank(code)) {
			DivisionVO vo = divisionTransport.getByCode(code);
			return ResponseVO.getResponseSuccess(vo);
		}
		return ResponseVO.getResponseError("请填写信息");
	}
	 ...
	 * @throws Exception
	 */
	@ApiOperation(value = "保存行政区划信息", tags = "保存行政区划信息", httpMethod = "POST", protocols = "application/json")
	@PostMapping("/save")
	public ResponseVO save(@RequestBody DivisionVO vo) throws Exception {
		...
		if (personnelVO != null) {
			if (vo != null && StrUtil.isBlank(vo.getName()) && StrUtil.isBlank(vo.getCode())) {
				if (vo.getSuperior() == null || divisionTransport.getByCode(vo.getSuperior().getCode()) != null) {
					if (divisionTransport.getByCode(vo.getCode()) == null) {
						if (divisionTransport.save(vo)) {
	 ...
	 * @throws Exception
	 */
	@ApiOperation(value = "修改行政区划信息", tags = "修改行政区划信息", httpMethod = "POST", protocols = "application/json")
	@PostMapping("/update")
	public ResponseVO update(@RequestBody DivisionVO vo) throws Exception {
		...
		if (personnelVO != null) {
			if (vo != null && StrUtil.isBlank(vo.getId())) {
				if (vo.getSuperior() == null || divisionTransport.getByCode(vo.getSuperior().getCode()) != null) {
					if (divisionTransport.getByCode(vo.getCode()) == null || divisionTransport.getByCode(vo.getCode()).getCode().equals(vo.getCode())) {
						if (divisionTransport.update(vo)) {
