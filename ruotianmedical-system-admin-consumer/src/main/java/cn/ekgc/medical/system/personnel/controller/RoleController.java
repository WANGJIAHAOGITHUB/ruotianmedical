 ...
 * @since 1.0.0
 */
@Api(value = "若天医疗系统信息 - 角色信息控制层", tags = "若天医疗系统信息 - 角色信息控制层")
@RestController("roleController")
@RequestMapping("/system/personnel/role")
public class RoleController extends BaseController {
	...
	public ResponseVO getListByPage(@PathVariable("pageNum") Integer pageNum, @PathVariable("pageSize") Integer pageSize, @RequestBody RoleVO vo) throws Exception {
		// 先将参数封装为为一个传输对象
		QueryPageVO<RoleVO> queryPageVO = new QueryPageVO<>(vo, pageNum, pageSize);
		// 调用传输层传入后端
		PageVO<RoleVO> pageVO = roleTransport.queryByPage(queryPageVO);
	...
	public ResponseVO getById(@PathVariable("id") String id) throws Exception {
		if (!StrUtil.isBlank(id)) {
			RoleVO vo = roleTransport.getById(id);
	...
	public ResponseVO getByCode(@PathVariable("code") String code) throws Exception {
		if (!StrUtil.isBlank(code)) {
			RoleVO vo = roleTransport.getByCode(code);
			return ResponseVO.getResponseSuccess(vo);
		}
		return ResponseVO.getResponseError("请填写信息");
	}
	...
	public ResponseVO save(@RequestBody RoleVO vo) throws Exception {
		...
		if (personnelVO != null) {
			if (vo != null && StrUtil.isBlank(vo.getName()) && StrUtil.isBlank(vo.getCode())) {
				if (roleTransport.getByCode(vo.getCode()) == null) {
					if (roleTransport.save(vo)) {
